(function(){
  ...
  var chapter=document.getElementById('postit-chapter');
  ...
  function mediumLayout(){var pad=6,gap=22;var langPos={x:pad,y:38};var articleX=pad+14;var langHeight=lang?lang.offsetHeight:248;var articleWidth=(article?article.offsetWidth:260)+14;var leftWidth=Math.max(lang?lang.offsetWidth:270,articleWidth);var rightWidth=Math.max(book?book.offsetWidth:260,chapter?chapter.offsetWidth:260);var rightX=Math.max(pad+leftWidth+28,Math.round(bounds.w-rightWidth-pad));var bookY=72;var bookHeight=book?book.offsetHeight:320;return {lang:langPos,book:{x:rightX,y:bookY},article:{x:articleX,y:langPos.y+langHeight+gap},chapter:{x:rightX+6,y:bookY+bookHeight+gap}};}
  function desktopLayout(){var pad=6,gap=22;var langPos={x:pad,y:38};var langHeight=lang?lang.offsetHeight:248;var articleY=langPos.y+langHeight+gap;var chapterX=Math.max(pad,Math.round(bounds.w-278));var chapterY=52;var bookX=Math.max(pad+300,Math.min(chapterX-292,Math.round(bounds.w*0.49)-130));return {lang:langPos,book:{x:bookX,y:72},article:{x:pad+14,y:articleY},chapter:{x:chapterX,y:chapterY}};}
  ...
  function applyMobileLayout(){
    ...
    if(bounds.w<720){
      ...
      posts.forEach(function(p){
        if(!p.el)return;
        if(p.key==='fish'){
          if(loaded.fish&&typeof loaded.fish.y==='number')pos.fish.y=loaded.fish.y;
          else pos.fish.y=desktopFishY(mediumDefaults);
          fishState.anchorY=pos.fish.y;
        }else{
          pos[p.key]=mediumDefaults[p.key];
        }
      });
      if(clown){
        if(loaded.clown&&typeof loaded.clown.x==='number'&&typeof loaded.clown.y==='number')pos.clown={x:loaded.clown.x,y:loaded.clown.y};
        else pos.clown=desktopClownLayout(mediumDefaults);
      }
      if(violet){
        if(loaded.violet&&typeof loaded.violet.x==='number'&&typeof loaded.violet.y==='number')pos.violet={x:loaded.violet.x,y:loaded.violet.y};
        else pos.violet=desktopVioletLayout(mediumDefaults);
      }
      if(green){
        if(loaded.green&&typeof loaded.green.x==='number'&&typeof loaded.green.y==='number')pos.green={x:loaded.green.x,y:loaded.green.y};
        else pos.green=desktopGreenLayout(mediumDefaults);
      }
      if(pawn){
        var mediumPawn=desktopPawnLayout();
